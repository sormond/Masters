...
java -jar ~/picard.jar BedToIntervalList I=WES_kit_target.bed O=$WES_target_interval SD=$reference_genome.dict ~/MH/reference/GATK_Bundle/bwa/Homo_sapiens_assembly38.dict

## picard CollectHsMetrics for coverage measurements
java -jar ~/picard.jar CollectHsMetrics I=$sample.bam O=$sample_output_hs_metrics.txt R=$reference.fasta BAIT_INTERVALS=$WES_target_interval TARGET_INTERVALS=$WES_target_interval
